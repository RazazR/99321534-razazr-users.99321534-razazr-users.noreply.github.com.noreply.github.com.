...
-----------------------
[BSON](http://bsonspec.org) still comes with a similar overhead. As of [the specification](http://bsonspec.org/#/specification), BSON also includes all the keys as cstrings and it also does not efficiently store integer or long values as there is no varint encoding. Looking on the hello world example should be self explaining: `{"hello":"world"}` (17 bytes) -> `<16 00 00 00 02 h e l l o 00 06 00 00 00 w o r l d 00 00>` (22 bytes)
